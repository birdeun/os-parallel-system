...
void run_compressor(ProcessLoad* my_bucket, int thread_count);
...
void run_compressor(ProcessLoad* my_bucket, int thread_count) {
    ...
    task_target = my_bucket->count;
    for (int i = 0; i < my_bucket->count; i++) {
        int idx = my_bucket->indices[i];
        Task* task = malloc(sizeof(Task));
        strncpy(task->data, "content", sizeof(task->data));
        char name[32];
        snprintf(name, sizeof(name), "file_%02d", idx);
        task->name = strdup(name);
        task->stage = RAW;
        task->size = file_sizes[idx];
        enqueue_task(task);
    }
...
int main(int argc, char* argv[]) {
    ...
    ProcessLoad buckets[P];
    assign_files_greedy(P, buckets);

    for (int i = 0; i < P; i++) {
        pid_t pid = fork();
        if (pid == 0) {
            run_compressor(&buckets[i], T);
            exit(0);
        }
    }
    end_perf(&metrics, P);
